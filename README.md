...
| | 轨迹长度 | 拖尾点数 50 ~ 800 |
| | 显示轨迹 | 开关拖尾显示 |
| | 发散后自动重启 | 开启后系统发散时自动重启，不显示毁灭弹窗（默认关闭） |
...
> **提示：** 点击右上角的 ⚙ 按钮可以打开设置面板，实时调整所有参数。

## 增加文明存续时间的参数调整建议

通过调整设置面板中的参数，可以显著延长文明（行星）的存续时间。

### 效果最明显的 3 个参数

| 参数 | 当前值 | 建议值 | 原因 |
|------|--------|--------|------|
| 恒星3 质量 (mass3) | 7 | 10 | 三星质量不等是混沌加速的主因。等质量 (10/10/10) 能形成更对称的构型，显著延缓系统发散 |
| 约束力 (ky) | 0.5 | 2.0 | 约束力系数。这是行星的"安全绳"——值越大，行星被甩远后拉回的力越强 |
| 约束距离 (bz) | 2.0 | 1.2 | 约束力激活距离。越小意味着约束越早介入，行星被束缚在更紧的轨道内 |

### 次要但有帮助的参数

| 参数 | 当前值 | 建议值 | 原因 |
|------|--------|--------|------|
| 约束力指数 (exy) | 1.0 | 2.0 | 越大则行星越远时约束力增长越陡，防逃逸效果更强 |
| 软化距离 (ras) | 0.15 | 0.35 | 增大后近距离接近时不会产生极端加速度，减少数值爆炸 |
| 时间尺度 (timeScale) | 3.0 | 1.5 | 降低仿真速度 = 更小的有效积分步长 = 更少的能量漂移，数值稳定性更好 |
| 发散阈值 (DD) | 8.0 | 15.0 | 放大容忍范围，不那么容易触发重置 |
